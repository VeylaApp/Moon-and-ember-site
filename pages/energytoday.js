// This code fetches today's astrological highlights and energies from a Supabase database, displaying them in a React component. It handles loading states, errors, and formats the data for presentation.
...
export default function TodaysHighlights() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const astroUrl = `https://bohmreonbrxneehnqvku.supabase.co/rest/v1/daily_astro_api?date=eq.${today}`;
        ...
        // Fetch ALL energy rows
        const energiesUrl = `https://bohmreonbrxneehnqvku.supabase.co/rest/v1/daily_energies?date=eq.${today}`;
  ...
  if (!astro || !astro.output) return <p className="text-red-400">No astrology data available for today.</p>;
  ...
  return (
    <Layout>
      <div className="max-w-3xl mx-auto p-6 bg-forest/30 text-white rounded-xl shadow-lg backdrop-blur border border-white/20">
        <h1 className="text-3xl font-header text-orange-ember mb-4">Today’s Energetic Highlights</h1>
      ...
      {new Date(`${dataDate}T12:00:00`).toLocaleDateString(undefined, {
        ...
        <section className="mb-6">
          ...
          {retrogrades.length > 0 ? (
            ...
            <p className="text-sm text-green-200 mt-2">✨ All planets move direct today.</p>
          )}
